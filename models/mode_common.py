...
def mode_model(city):
    ...
    writer = pd.ExcelWriter('../outputs/ML_Results/mode_MNLR/'  + city + '.xlsx', engine='openpyxl')
    ...
    HPO_summary.to_csv('../outputs/ML_Results/' + city + '_HPO_mode_common_summary.csv',index=False)
    ...
    with open('../outputs/ML_Results/shap/mode_common/' + city + '_importance.pkl', 'wb') as g:
    ...
    with open('../outputs/ML_Results/shap/mode_common/' + city + '.pkl', 'wb') as f:
    ...
    plt.savefig('../outputs/ML_Results/result_figures/mode_common/' + city + '_mode_CM.png',facecolor='w',dpi=65,bbox_inches='tight')
    plt.close() 
