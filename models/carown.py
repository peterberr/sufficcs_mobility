...
def carown_model(city):
    ...
    means_df.to_csv('../outputs/ML_Results/carown_LR/'  + city + '_mean.csv',index=False)
    ...
    HPO_summary.to_csv('../outputs/ML_Results/' + city + '_HPO_carown_summary.csv',index=False)

    # save shap_values, to enable later re-creation and editing of shap plots
    with open('../outputs/ML_Results/shap/carown/' + city + '.pkl', 'wb') as f:
    ...
    with open('../outputs/ML_Results/shap/carown/' + city + '_importance.pkl', 'wb') as f:
    ...
    plt.savefig('../outputs/ML_Results/result_figures/carown/' + city + '_FI_carown.png',facecolor='w',dpi=65,bbox_inches='tight')
    plt.close()
